...
$$
\rho_{ij} = \max_{m \in M} \left[ \right]
$$

$$
\frac{M}{d_\textrm{max}} \sum_k f(\mathbf{r}_{ik}, \mathbf{r}_{ij}) \int_{(m-1)d_{\textrm{max}}/M}^{ md_{\textrm{max}} /M } \textrm{d}x K\left( x - \mathbf{r}_{ks} \cdot \mathbf{r}_{ij}} / | \mathbf{r}_{ks} | \right) 
$$
